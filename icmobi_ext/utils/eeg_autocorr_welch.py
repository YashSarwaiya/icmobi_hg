...
def eeg_autocorr_welch(EEG, pct_data=100):
    ...
    #HG - 1/25/2025 - cast n_points to int
    n_points = int(min(EEG['pnts'], EEG['srate'] * 3))
    ...
    if EEG['pnts'] < EEG['srate']:
        ...
        #HG - 1/25/2025 - cast EEG['srate'] to int to fix slice indices
        ac = ac[:, :int(EEG['srate']) + 1] / (ac[:, 0][:, np.newaxis] * np.concatenate((np.arange(n_points, n_points - EEG['srate'], -1), np.array([max(1, n_points - EEG['srate'])]))) / n_points)
