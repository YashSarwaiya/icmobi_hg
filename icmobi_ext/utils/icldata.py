...
#(09/16/2024) JS, trying solution from thread: https://forum.hdfgroup.org/t/memory-leak-while-reading-data/9473/3
# by using gc (garbage collector)
import gc
...
class ICLabelDataset:
    ...
    * Hand crafted features (some new and some from previously published classifiers)
    ...
    def __init__(self,
        ...
        :param n_val_ics: How many labeled components to transfer to the validdation set.
        ...
        # self.train_feature_indices = OrderedDict([
        #     ('ids', np.arange(2)),
        #     ('topo', np.random.randint(2, 742, 10)),
        #     ('handcrafted', np.random.randint(742, 760,10)),  # one lost due to removal in load_data
        #     ('dipole', np.random.randint(760, 780, 10)),
        #     ('psd', np.random.randint(780, 880,10)),
        #     ('psd_var', np.random.randint(880,980,10)),
        #     ('psd_kurt', np.random.randint(980, 1080,10)),
        #     ('autocorr', np.random.randint(1080, 1180,10)),
        # ])
        ...
        # self.n_test_datasets_feature_indices = OrderedDict([
        #     ('ids', np.arange(3)),
        #     ('topo', np.random.randint(3, 743, 10)),
        #     ('handcrafted', np.random.randint(743, 761,10)),  # one lost due to removal in load_data
        #     ('dipole', np.random.randint(761, 781, 10)),
        #     ('psd', np.random.randint(781, 881,10)),
        #     ('psd_var', np.random.randint(881,981,10)),
        #     ('psd_kurt', np.random.randint(981, 1081,10)),
        #     ('autocorr', np.random.randint(1081, 1181,10)),
        # ])
    ...
    def __load_matlab_cellstr(f, var_name=''):
        # loads in cells of strings data from a .mat file
        var = []
        if var_name:
            for column in f[var_name]:
                row_data = []
                for row_number in range(len(column)):
                        # (08/15/2024) JS, unichr depricated replacing with chr
                        # (08/22/2024) JS, Typerror: only integer scalar arrays... 
                        # fixed by using str instead of chr in map function
                        # (09/19/2024) JS, fixed an issue where str was converting numbers to 
                        # integer literals. using loop now, sadly.
                    # row_data.append(''.join(map(str, f[column[row_number]][:])))
                    # row_data.append(''.join(map(chr, f[var_name][column][row_number][:])))
                    tmp = np.array(f[column[row_number]])
                    for i in tmp:
                        row_data.append(chr(i[0]))
                    row_data = ''.join(row_data)
                var.append(row_data)
                # return [str(x)[3:-2] for x in var]
        return [str(x) for x in var]
    ...
    def load_data(self):
        ...
        # if not, create it
            ...
            with h5py.File(join(self.datapath, 'features', 'features_0D1D2D.mat'), 'r') as f:     
                ...
                #9/16/2024 - HG - subsetting arrays to prevent mem overflow
                #(11/09/2024) JS, temporarily disabling subseting to see if it's the cause of the bug
                # keys = np.random.randint(0, 2416738, 200)
                # features.append(np.asarray(f['features']).T[keys, :])
                features.append(np.asarray(f['features']).T)
                feature_labels.append(self.__load_matlab_cellstr(f, 'labels'))
            #(09/16/2024) JS, garabge collector to help clean up memory leaks from h5py loads (try 1)
            gc.collect()
            ...
            with h5py.File(join(self.datapath, 'features', 'features_PSD_med_var_kurt.mat'), 'r') as f:
                ...
                for element in f['features_out'][0]:
                    ...
                    if data.ndim == 1 or data.dtype != np.float64:
                        continue
                    #(09/16/2024) JS, int fix might not be appropriate here. I think it was more of a bug with h5py
                    # loading schema. https://stackoverflow.com/questions/46733052/read-hdf5-file-into-numpy-array
                    # followup, it is the right fix, need to int to get the range function to work.
            ...
            if self.label_type == 'database':
                ...
                with open(join(self.datapath, 'labels', file_name), 'rb') as f:
                    ...
                    # HG 9/17/24 -> implemented JS solution, specify latin1 encoding for python3 pickle load method
            ...
            #HG - 9/17/2024 - fixed deprecated np.int cast
            temp = self.__match_indices(label_index.astype(int), features[0][:, :2].astype(int))
            ...
            #HG - 9/17/2024 - fixed deprecated np.int cast
            feature_inds = self.__match_indices(*[x[:, :2].astype(int) for x in features])

            # check which labels are not kept
            print('Rearanging components and labels...')
            # (08/15/2024) JS, iteritems depricated
            ...
            #HG 9/18/2024 - Need JS advisory on labelling conventions. What is the equiv to 'number of data points' in our labels?
            ...
            print('Rematching components and labels...')
             #9/20/2024 HG -> fixed deprecated dtype np.int cast
            ind_labeled_labels, ind_labeled_features = self.__match_indices(
                label_index.astype(int),features[0][feature_inds[0], :2].astype(int))
            del label_index
            ...
            #9/20/2024 - HG - added line to create dir, solves 1281 no dir found bug
            os.makedirs(os.path.dirname(join(self.datapath, 'cache', processed_file_name)), exist_ok=True)
    ...
    def _load_classifications(self, ids=None):
        ...
        with h5py.File(join(self.datapath, 'other', 'other_classifiers.mat'), 'r') as f:
            ...
            # (08/15/2024) JS, iteritems() was removed in python 3
            # for cls, lab in f.iteritems():
            #     classifications[cls] = lab[:].T

        # match to given ids
        if ids is not None:
            # (08/15/2024) JS, iteritems is depricated
    ...
    def _plot_grid(data, function):
        nax = data.shape[0]
        #9/20/2024 HG -> fixed deprecated dtype np.int cast
        a = np.ceil(np.sqrt(nax)).astype(int)
        b = np.ceil(1. * nax / a).astype(int)
