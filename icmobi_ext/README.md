...
# Preparing for HPG Training
create new virtual enviornment via conda:

module load conda
conda create -n torch_cpu python=3.11
conda activate torch_cpu

## (Preferred) Install packages using setup.py
navigate to python_src folder (cd ./path/to/python_src)
pip install --upgrade pip setuptools wheel
pip isntall -e .

# Conda installation, repo reconfiguration may be required.
conda install numpy pandas tqdm matplotlib -c conda-forge
